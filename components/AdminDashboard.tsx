...
const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'inventory' | 'analytics' | 'staff' | 'vendors' | 'products' | 'users'>('orders');
  ...
  const fetchData = async () => {
    try {
      const [ordersRes, invRes, anaRes, staffRes, vendorsRes, statsRes, usersRes] = await Promise.all([
      ...
      <div className="w-full md:w-64 bg-coffee-900 text-white p-6 space-y-8">
        ...
        <nav className="space-y-2">
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'users' ? 'bg-coffee-600 shadow-lg' : 'hover:bg-white/10'}`}
          >
                ...
                <tbody className="divide-y divide-slate-100">
                  {users.map((u) => (
                    <tr key={u.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <select 
                          className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-xs font-medium focus:ring-1 focus:ring-coffee-500 outline-none"
                          value={u.role}
