          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {sortedInventory.map((item) => {
                const currentStockValue = editingStock[item.id] !== undefined ? parseInt(editingStock[item.id]) : item.stock;
                const displayValue = editingStock[item.id] !== undefined ? editingStock[item.id] : item.stock.toString();
                
                return (
                  <div key={item.id} className={`p-6 rounded-2xl border ${currentStockValue <= item.minStock ? 'bg-red-50 border-red-100' : 'bg-white border-slate-100'}`}>
                    <div className="flex justify-between items-start mb-6">
                      <h4 className="font-bold text-slate-900">{item.name}</h4>
                      {currentStockValue <= item.minStock && (
                    ...
                    <div className="space-y-4">
                      <div>
                        <p className="text-xs text-slate-500 uppercase font-bold mb-3">Current Stock</p>
                        <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl p-2">
                          <button 
                            onClick={() => {
                              const newVal = Math.max(0, currentStockValue - 1);
                              setEditingStock({...editingStock, [item.id]: newVal.toString()});
                              updateStockDirect(item.id, newVal);
                            }}
                            className="w-10 h-10 flex items-center justify-center bg-white border border-slate-300 rounded-lg hover:bg-slate-100 active:bg-slate-200 transition-all shadow-sm font-bold text-lg text-slate-600"
                          >
                            −
                          </button>
                          <input 
                            ...
                            onChange={(e) => {
                              ...
                              if (val === '' || val === '-') {
                                setEditingStock({...editingStock, [item.id]: val});
                              } else {
                            ...
                            onBlur={(e) => {
                              const val = e.target.value;
                              if (val === '' || val === '-') {
                                setEditingStock({...editingStock, [item.id]: item.stock.toString()});
                              } else {
                                const num = parseInt(val);
                                if (!isNaN(num)) {
                                  const clampedVal = Math.max(0, Math.min(999, num));
                                  updateStockDirect(item.id, clampedVal);
                                  setEditingStock({...editingStock, [item.id]: undefined});
                                }
                              }
                            }}
                            className="flex-1 text-2xl font-bold text-slate-900 bg-transparent border-none focus:ring-0 focus:outline-none text-center py-2"
                            placeholder="0"
                          />
                          <button 
                            onClick={() => {
                              const newVal = Math.min(999, currentStockValue + 1);
                              setEditingStock({...editingStock, [item.id]: newVal.toString()});
                              updateStockDirect(item.id, newVal);
                            }}
                            className="w-10 h-10 flex items-center justify-center bg-coffee-600 text-white rounded-lg hover:bg-coffee-700 active:bg-coffee-800 transition-all shadow-sm font-bold text-lg"
                          >
                            +
                          </button>
                        </div>
                      ...
                      <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
                        <div 
                          className={`h-full transition-all ${currentStockValue <= item.minStock ? 'bg-red-500' : 'bg-emerald-500'}`}
                          style={{ width: `${Math.min(100, (currentStockValue / (item.minStock * 3)) * 100)}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-4">
              ...
              {newMenuItem.imageUrl && (
                <div className="h-32 w-full rounded-xl overflow-hidden border border-slate-200">
                  <img src={newMenuItem.imageUrl} alt="Preview" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                </div>
              )}
              <button 
