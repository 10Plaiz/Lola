...
const OnboardingModal: React.FC = () => {
  ...
  const handleVerifyAndSetPassword = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await completeOnboarding(user.email, otp, password, phone);
    } catch (err: any) {
            ...
            <div className="space-y-6 text-center">
              <div className="p-4 bg-coffee-50 rounded-2xl border border-coffee-100">
                <p className="text-coffee-700 text-sm leading-relaxed">
                  We'll send a verification code to <span className="font-bold text-coffee-900">{user.email}</span>.
                </p>
                <p className="text-[10px] text-coffee-400 mt-2 italic">
                  Note: If you receive a link instead of a code, click the link in your email then return here.
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleVerifyAndSetPassword} className="space-y-6">
              <div className="space-y-2">
                <div className="flex justify-between items-end ml-1">
                  <label className="text-xs font-bold text-coffee-700 uppercase tracking-widest">Verification Code</label>
                  <button 
                    type="button"
                    onClick={() => setOtp('LINK_VERIFIED')}
                    className="text-[10px] text-coffee-500 hover:text-coffee-800 underline"
                  >
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    required
                    value={otp === 'LINK_VERIFIED' ? '' : otp}
                    disabled={otp === 'LINK_VERIFIED'}
                    onChange={(e) => setOtp(e.target.value)}
                    className="w-full pl-12 pr-4 py-3 bg-white border border-coffee-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-coffee-500 text-coffee-900 placeholder-coffee-300 transition-all disabled:bg-coffee-50 disabled:text-coffee-400"
                    placeholder={otp === 'LINK_VERIFIED' ? 'Verified via Link' : 'Enter 6-digit code'}
                  />
                </div>
                <p className="text-[10px] text-coffee-500 ml-1">
                  {otp === 'LINK_VERIFIED' ? 'Great! Now just set your password below.' : 'Check your email for the code or link.'}
                </p>
              </div>
              ...
              <div className="space-y-4 pt-2">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-coffee-400 font-bold text-sm">+63</div>
                    <input
                      type="tel"
                      required
                      value={phone}
                      onChange={(e) => setPhone(e.target.value.replace(/\D/g, '').slice(0, 10))}
                      className="w-full pl-14 pr-4 py-3 bg-white border border-coffee-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-coffee-500 text-coffee-900 placeholder-coffee-300 transition-all"
